...
def plot_quadrotor(shooting_nodes, u_max, U, X_true, X_est=None, Y_measured=None, latexify=False, plt_show=True, X_true_label=None):
    """
    Params:
    ...
    # potřeba vylepšit prinťění
    U_0 = U[:,0]
    line, = plt.step(t, np.append([U_0[0]], U_0))
    ...
    states_lables = ['$x$', '$y$', '$z$', '$v_x$', '$v_y$', '$v_z$']
...
def yaml_parse(dict, key, default = None):
    if key in dict.keys():
        return dict[key]
    else:
        print("This key was not found: " + key + ". Using default value. ")
        return default
